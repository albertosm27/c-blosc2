...
void print_compress_info(void) {
  ...
  if (ret >= 0) printf("  %s: %s\n", name, version);
  ret = blosc_get_complib_info("lz5", &name, &version);
  if (ret >= 0) printf("  %s: %s\n", name, version);
...
int main(int argc, char* argv[]) {
  ...
  strncpy(usage, "Usage: bench [blosclz | lz4 | lz4hc | lz5 | lz5hc | snappy | zlib | zstd] "
  ...
  if (strcmp(compressor, "blosclz") != 0 &&
      ...
      strcmp(compressor, "lz5") != 0 &&
      strcmp(compressor, "lz5hc") != 0 &&
