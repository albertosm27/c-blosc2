 ...
 * compressed block. The size of @p src buffer is specified by
 * @p nbytes. There is not a minimum for @p src buffer size (@p nbytes).
 *
 ...
 *
 * Compression is memory safe and guaranteed not to write the @p dest
 ...
 *
 * @a BLOSC_CLEVEL=(INTEGER): This will overwrite the @p clevel parameter
 * before the compression process starts.
 *
 * @a BLOSC_SHUFFLE=[NOSHUFFLE | SHUFFLE | BITSHUFFLE]: This will
 * overwrite the @p doshuffle parameter before the compression process
 * starts.
 *
 * @a BLOSC_DELTA=(1|0): This will call blosc_set_delta() before the
 * compression process starts.
 *
 * @a BLOSC_TYPESIZE=(INTEGER): This will overwrite the @p typesize
 * parameter before the compression process starts.
 *
 * @a BLOSC_COMPRESSOR=[BLOSCLZ | LZ4 | LZ4HC | LIZARD | SNAPPY | ZLIB]:
 * This will call blosc_set_compressor(BLOSC_COMPRESSOR) before the
 * compression process starts.
 *
 * @a BLOSC_NTHREADS=(INTEGER): This will call
 * blosc_set_nthreads(BLOSC_NTHREADS) before the compression process
 * starts.
 *
 * @a BLOSC_BLOCKSIZE=(INTEGER): This will call
 ...
 *
 * @a BLOSC_NOLOCK=(ANY VALUE): This will call #blosc2_compress_ctx under
 * the hood, with the @p compressor, @p blocksize and
 * @p numinternalthreads parameters set to the same as the last calls to
 * #blosc_set_compressor, #blosc_set_blocksize and
 * #blosc_set_nthreads. @a BLOSC_CLEVEL, @a BLOSC_SHUFFLE, @a BLOSC_DELTA and
 * @a BLOSC_TYPESIZE environment vars will also be honored.
 */
 ...
 * The @p src buffer and the @p dest buffer can not overlap.
 *
 ...
 * process starts.
 *
 * @a BLOSC_NOLOCK=(ANY VALUE): This will call #blosc2_decompress_ctx
 * under the hood, with the @p numinternalthreads parameter set to the
...
/**
 * @brief Find whether the frame has a namespace or not.
 *
 * @param frame The frame from which the namespace will be checked.
 * @param name The name of the namespace to be checked.
 *
 * @return If successful, return the index of the namespace. Else, return a negative value.
 */
BLOSC_EXPORT int blosc2_frame_has_namespace(blosc2_frame *frame, char *name);

/**
 * @brief Add content into a new namespace.
 *
 * @param frame The frame to which add the namespace.
 * @param name The name of the namespace.
 * @param content The content of the namespace.
 * @param content_len The length of the content.
 *
 * @return If successful, the index of the new namespace. Else, return a negative value.
 */
BLOSC_EXPORT int blosc2_frame_add_namespace(blosc2_frame *frame, char *name, uint8_t *content,
                                            uint32_t content_len);

/**
 * @brief Update the content of an existing namespace.
 *
 * @param frame The frame containing the namespace.
 * @param name The name of the namespace to be updated.
 * @param content The new content of the namespace.
 * @param content_len The length of the content.
 *
 * @return If successful, the index of the namespace. Else, return a negative value.
 */
BLOSC_EXPORT int blosc2_frame_update_namespace(blosc2_frame *frame, char *name, uint8_t *content,
                                               uint32_t content_len);

/**
 * @brief Get the content out of a namespace.
 *
 * @param frame The frame containing the namespace.
 * @param name The name of the namespace.
